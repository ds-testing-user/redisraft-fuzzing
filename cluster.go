...
func (c *ClusterConfig) SetDefaults() {
	if c.RaftModulePath == "" {
		c.RaftModulePath = "/Fuzzing/redisraft-fuzzing/redisraft.so"
	}
	if c.RedisServerBinaryPath == "" {
		c.RedisServerBinaryPath = "/Fuzzing/redis/src/redis-server"
	}
	if c.WorkingDir == "" {
		c.WorkingDir = "/Fuzzing/redisraft-fuzzing/tests/tmp"
	}
